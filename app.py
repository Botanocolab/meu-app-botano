...
# 2. Carregar Dados
@st.cache_data(ttl=60) # Cache para evitar chamadas excessivas à API
...
if not df.empty and 'valor_aposta' in df.columns:
    ...
    if not sugestoes.empty:
        for i, row in sugestoes.iterrows():
            ...
            col1.info(f"Oportunidade: **{row['evento']}** | {row['time_casa']} | Odd: {row['odd_casa']} (Valor: +{row['valor_aposta']:.2f})")
        ...
        st.write("Nenhuma sugestão de valor alto encontrada no momento.")
else:
    st.write("Aguardando dados de mercado...")

# 4. Jogos Disponíveis + Filtro de Casas (O que você pediu)
...
if not df.empty:
    ...
    st.error("Nenhum dado encontrado na API.")

# 5. Simulador
st.divider()
st.subheader("🎯 Simulador")
with st.form("sim_form"):
    evento = st.selectbox("Jogo:", df['evento'].unique() if not df.empty else ["Nenhum"])
